    ...
    const renderFieldHTML = (field: RenderedField, index: number) => {
        switch (field.element) {
            ...
            case 'button_next_step':
                return (
                    <StyledField key={index}>
                        <StyledButton name={field.attrs.name} type="button" onClick={() => setCurrentStep(currentStep+1)}>{field.attrs.value}</StyledButton>
                    </StyledField>
                )
            case 'button_previous_step':
                return (
                    <StyledField key={index}>
                        <StyledButton name={field.attrs.name} type="button" {...field.attrs} onClick={() => setCurrentStep(currentStep-1)}>{field.attrs.value}</StyledButton>
                    </StyledField>
                )
            default:
                return <div></div>
...
export async function getStaticProps() {
    const steps = await renderForm('yorrics')
